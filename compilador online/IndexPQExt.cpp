...
template <typename L, typename T, typename Comparator = std::less<T>>
class IndexPQ {
public:
...
private:
   ...
   std::unordered_map<L, int> posiciones;   // un 0 indica que el elemento no está
...
public:
   ...
   IndexPQ(IndexPQ<L, T,Comparator> const&) = default;
   
   IndexPQ<L, T,Comparator>& operator=(IndexPQ<L, T,Comparator> const&) = default;
   
   ~IndexPQ() = default;
   
   // e debe ser uno de los posibles elementos
   void push(L e, T const& p) {
...
int main() {
   /* descomenta este código para probar tu solución*/
