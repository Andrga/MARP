...
public:
   ...
   /**
    * Devuelve si hay una arista que conecta dos vertices dados.
    * @throws domain_error si v o w no existe.
    * Complejidad del algoritmo lineal en el numero de adyacentes en v.
    */
    bool const hayArista(int v, int w){
      if (v < 0 || v >= _V || w < 0 || w >= _V)
         throw std::domain_error("Vertice inexistente");
        for(int vertice: _ady[v])
            if (vertice == w)
                return true;        
        return false;
    }
