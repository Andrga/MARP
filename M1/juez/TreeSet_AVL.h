...
protected:
	...
	struct TreeNode {
		...
		Link iz, dr;
		int altura;
		// numero de nodos hijos en la izquierda +1
		int tam_i;
...
protected:
	...
	bool inserta(T const& e, Link& a) {
		...
		if (a == nullptr) { // se inserta el nuevo elemento e
			...
			if (crece) {
				a->tam_i++; // si se inserta nodo a la izquierda aumenta el tamanyo en 1
				reequilibraDer(a);
			}
		}
		else if (menor(a->elem, e)) {
	...
	void rotaDer(Link& r2) {
		...
		r2->tam_i = size(r2) - size(r1); // recalcula el tamanio del nodo raiz
	...
	int size(Link a) const
	{
		return a != nullptr ? a->tam_i : 0;
	}
