...
public:
	Inecuacion(Digrafo& g) : visit(g.V(), false), apilado(g.V(), false), ciclo(false)
	{
		for (int i = 0; i < g.V(); i++)
			if (!visit[i])
				dfs(g, i);
...
bool resuelveCaso() {
	...
	Inecuacion inecuancion(g);

	if (inecuancion.getCiclo()) cout << "NO";
	else {
		cout << "SI ";
		deque<int> n = inecuancion.getOrden();
